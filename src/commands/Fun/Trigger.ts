...
import { ISimplifiedMessage, IParsedArgs } from '../../typings/index.js'
    ...
    run = async (M: ISimplifiedMessage): Promise<void> => {
        const getImage = async (image: string | Buffer, timeout = 15) => {
            ...
            for (let i = 0; i < 9; i++) {
                ...
                ctx.drawImage(
                    await loadImage(this.client.assets.get('triggered') || Buffer.from('')),
        ...
        try {
            ...
            const sticker = new Sticker(await getImage(image), {
                pack: `Triggered`,
                author: M.sender.username || `Ari-Ani`,
                type: 'full',
                categories: ['💢']
            })
