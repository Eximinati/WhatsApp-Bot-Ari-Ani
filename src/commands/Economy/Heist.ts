...
export default class Command extends CommandModule {
    ...
    run = async (M: ISimplifiedMessage, { joined }: IParsedArgs): Promise<void> => {
        ...
        try { const res = await economy.heist(M.sender.jid, tid); const cv = createCanvas(W, H), ctx = cv.getContext('2d'); const g = ctx.createLinearGradient(0, 0, 0, H); g.addColorStop(0, BG1); g.addColorStop(1, BG2); ctx.fillStyle = g; ctx.fillRect(0, 0, W, H)
