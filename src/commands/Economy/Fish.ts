...
const BG1 = '#001a26', BG2 = '#002d40', AC = '#00bcd4'
const economy = new EconomyService()

export default class Command extends CommandModule {
    constructor(client: RuntimeClient, handler: MessagePipeline) {
        super(client, handler, { command: 'fish', description: 'Go fishing for coins', category: 'economy', usage: `${client.config.prefix}fish`, aliases: ['fishing'], baseXp: 12 })
    }
    run = async (M: ISimplifiedMessage, _: IParsedArgs): Promise<void> => {
        try {
            const res = await economy.fish(M.sender.jid)
            ...
            if (res.ok && res.success) {
                ...
                ctx.fillStyle = '#ffc107'; ctx.font = 'bold 28px "Segoe UI", "Arial", sans-serif'; ctx.fillText('The fish got away...', W / 2, 170)
            }

            ctx.fillStyle = 'rgba(255,255,255,0.2)'; ctx.font = '12px "Segoe UI", "Arial", sans-serif'; ctx.fillText('Ari-Ani Economy  •  /fish to try again', W / 2, H - 16)
