...
const BG1 = '#0f2027', BG2 = '#203a43', AC = '#4cd964'
...
export default class Command extends CommandModule {
    constructor(client: RuntimeClient, handler: MessagePipeline) {
        super(client, handler, { command: 'work', description: 'Work a shift to earn coins', category: 'economy', usage: `${client.config.prefix}work`, aliases: ['job', 'shift'], baseXp: 20 })
    }
    run = async (M: ISimplifiedMessage, _: IParsedArgs): Promise<void> => {
        try {
            ...
            ctx.fillText('💼 WORK SHIFT', W / 2, 60)
            if (res.ok && res.success) {
                ...
                ctx.fillText('You finished a focused shift.', W / 2, 215)
                ...
                ctx.fillStyle = '#ff6b6b'; ctx.font = 'bold 32px "Segoe UI",sans-serif'
                ctx.fillText('⏳ Cooldown', W / 2, 160)
                ctx.fillStyle = 'rgba(255,255,255,0.55)'; ctx.font = '18px "Segoe UI",sans-serif'
                ctx.fillText(`Wait ${formatDurationMs(res.remainingMs || 0)}`, W / 2, 210)
            } else {
                ctx.fillStyle = 'rgba(255,107,107,0.12)'; rr(ctx, 80, 90, 520, 160, R); ctx.fill()
                ctx.fillStyle = '#ff6b6b'; ctx.font = 'bold 32px "Segoe UI",sans-serif'
                ctx.fillText('❌ Work failed', W / 2, 160)
            }
            ctx.fillStyle = 'rgba(255,255,255,0.25)'; ctx.font = '12px "Segoe UI",sans-serif'
            ...
            return void M.reply(cv.toBuffer('image/png'), MessageType.image, Mimetype.png, undefined,
                res.ok && res.success
                    ? `💼 Work shift complete! +${formatMoney(res.reward || 0)} | Wallet: ${formatMoney(res.account.wallet)} | Bank: ${formatMoney(res.account.bank)}`
                    : res.reason === 'cooldown' ? `⏳ Cooldown — wait ${formatDurationMs(res.remainingMs || 0)}` : '❌ Work failed. Try again.')
