...
const DEFAULT_PATH = join(__dirname, '..', '..', 'assets', 'json', 'default-config.json')
const DEFAULT_CONFIG: ICharacter = JSON.parse(readFileSync(DEFAULT_PATH, 'utf8'))
...
export default class Identity {
    ...
    applyAdd = async (jid: string, kind: IdentityKind, add: IIdentityAdd | null | undefined): Promise<void> => {
        ...
        this.client.log(`PERSONA ${kind}:${jid} ${summary}`)
    }
    ...
    private sanitize = (entries: string[] | undefined, maxChars: number): string[] => {
        ...
        for (const raw of entries) {
            ...
            if (isGaslight(trimmed)) {
                this.client.log(
                    `Input filtered: "${trimmed.slice(0, 80)}"`,
                    true
                )
                continue
            }
    ...
    private merge = (delta: ICharacterDelta): ICharacter => ({
        ...
        lore: [...DEFAULT_CONFIG.lore, ...delta.lore],
        topics: [...DEFAULT_CONFIG.topics, ...delta.topics],
        style: {
            all: DEFAULT_CONFIG.style.all,
            chat: [...DEFAULT_CONFIG.style.chat, ...delta.styleChat],
            post: DEFAULT_CONFIG.style.post
        },
        adjectives: DEFAULT_CONFIG.adjectives,
        bio: DEFAULT_CONFIG.bio,
        name: DEFAULT_CONFIG.name,
        messageExamples: DEFAULT_CONFIG.messageExamples,
        postExamples: DEFAULT_CONFIG.postExamples
    })
